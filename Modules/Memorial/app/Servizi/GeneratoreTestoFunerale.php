 ...
 *   Parrocchia Immacolata Concezione, Boscoreale, NA
...
class GeneratoreTestoFunerale
{
    ...
    private function preparaDati(Defunto $defunto): array
    {
        return [
            ...
            'indirizzo' => $defunto->indirizzo_cerimonia,
            'chiesa' => $defunto->chiesa,
            'cimitero' => $defunto->cimitero,
            'citta' => $defunto->citta,
    ...
    private function promptDiSistema(): string
    {
        return <<<PROMPT
            ...
            Ricevi in JSON dati già puliti e verificati: quando (già risolto — "oggi", "domani" o "il [data estesa]": usalo esattamente così, non calcolarlo), ora, indirizzo, chiesa, cimitero, citta, provincia.
            ...
            {chiesa}, {citta}, {provincia}
            Cimitero, {citta}, {provincia}

            Regole:
            - Non inventare né alterare alcun dato fattuale (indirizzi, orari, nomi, città, provincia): riportali esattamente come ricevuti.
     ...
     * manuale "Info funerale" già esistente nel designer.
     */
    private function componiTemplate(array $dati): string
    {
        $luogo = collect([$dati['citta'], $dati['provincia']])->filter()->implode(', ');
        $conLuogo = fn (?string $base) => collect([$base, $luogo])->filter()->implode(', ');
        ...
        return implode("\n", [
            "I funerali si svolgeranno {$quando} alle ore {$ora}",
            'Partenza da '.$conLuogo($dati['indirizzo']),
            $conLuogo($dati['chiesa'] ?: 'Parrocchia'),
            $conLuogo('Cimitero'),
        ]);
    }
}
