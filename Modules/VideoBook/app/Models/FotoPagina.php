...
/**
 * Cosa c'è in un riquadro di una pagina: una foto.
 *
 * `slot` combacia con l'`ordine` dentro l'albero `PaginaTemplate::slots`
 * (risolto in x/y/w/h da Support\GrigliaPagina per un formato dato): è così
 * che si sa dove va questa foto, sia in stampa (jsPDF) sia nel video — una
 * scena per foto, non per pagina intera (Ken Burns per singola foto dentro
 * la pagina, come deciso per il render).
 *
...
class FotoPagina extends Model
{
    ...
    protected $fillable = ['pagina_id', 'slot', 'path', 'scala', 'pos_x', 'pos_y', 'durata_secondi', 'stile'];
